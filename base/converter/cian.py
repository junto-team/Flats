...
def append_bargain_rent(attrs, root_tag):
    ...
    client_fee = etree.SubElement(bargain_tag, 'ClientFee')
    client_fee.text = '0'

    agent_fee = etree.SubElement(bargain_tag, 'AgentFee')
    agent_fee.text = '0'
...
def generate_yrl(db):
    ...
    for content_id, content in db['content'].items():
        ...
        if not attrs.get('objectLoadCian', None):
            continue
        ...
        try:
            ...
            for i in images:
                url = 'https://mezonn.ru/forxml/{}'.format(i['image'])
